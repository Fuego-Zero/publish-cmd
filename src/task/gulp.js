...
function uglifyConfig(config) {
    return new Promise((resolve, reject) => {
        ...
        gulp.src([path.join(config.projectPath, config.src, 'config.js'), path.join(config.projectPath, config.src, 'init.js')])
            .pipe(ngAnnotate())
            .pipe(uglify())
            .pipe(rename(function (path) {
...
function modifyIndexmodifyIndex(config) {
    return new Promise((resolve, reject) => {
        ...
        let removeTempRefer = function () {
            let con = getIndexContent().replace(/\s*<script.*data-temp-file><\/script>/g, '');
            fs.writeFileSync(indexSrc, con);
        }();

        //增加index页面中的 app.js
        let addAppJs = function () {
            let con = getIndexContent().replace('</head>', '\t<script src="app.js"></script>\r\n' + '</head>');
            fs.writeFileSync(indexSrc, con);
        }();
        ...
        function changeIndexRefer(data) {
            ...
            let i, source, temp;
            let conMin = getIndexContent().replace(/(\.js["']|\.css["'])\s+data-nd-cp/g, '.min$1');
            ...
            for (i = 0; i < source.length; i++) {
                ...
                temp = '<script src="http://package.jjiehao.com' + source[i].replace(/^\./, '') + '"></script>';
                if (new RegExp(temp).test(conMin)) {
                    if (i === source.length - 1) {
                        conMin = conMin.replace(new RegExp(temp), '<script src="http://package.jjiehao.com' + data.js.dest.replace(/^\./, '') + '/' + data.js.newName + '"></script>');
                    } else {
            ...
            for (i = 0; i < source.length; i++) {
                ...
                temp = '<link rel="stylesheet" href="http://package.jjiehao.com' + source[i].replace(/^\./, '') + '">';
                if (new RegExp(temp).test(conMin)) {
                    if (i === source.length - 1) {
                        conMin = conMin.replace(new RegExp(temp), '<link rel="stylesheet" href="http://package.jjiehao.com' + data.css.dest.replace(/^\./, '') + '/' + data.css.newName + '">');
                    } else {
                        conMin = conMin.replace(new RegExp('\\s*' + temp), '');
                    }
                } else {
            ...
            conMin = conMin.replace(/src="http:\/\/package.jjiehao.com\/js\/(.*\.min\.js)/g, function (val, $1) {
                return val + '?v=' + data.md5Js[$1];
            });

            conMin = conMin.replace(/href="http:\/\/package.jjiehao.com\/css\/(.*\.min\.css)/g, function (val, $1) {
                return val + '?v=' + data.md5Css[$1];
            });

            //相对地址 css 的文件加hash值
            for (let i = 0; i < config.customMd5.css.length; i++) {
                conMin = conMin.replace('href="tpls/' + config.customMd5.css[i], 'href="tpls/' + config.customMd5.css[i] + '?v=' + crypto.createHash('md5').update(config.tpls + '/' + config.customMd5.css[i]).digest('hex').slice(0, 10));
            }

            //相对地址 js 的文件加hash值
            //需要加前缀 不然会与 ueditor.config.min.js 冲突
            for (let i = 0; i < config.customMd5.js.length; i++) {
                conMin = conMin.replace('src="' + config.customMd5.js[i], 'src="' + config.customMd5.js[i] + '?v=' + crypto.createHash('md5').update(config.src + '/' + config.customMd5.js[i]).digest('hex').slice(0, 10));
            }
...
function publish(config) {
    ...
    execPublish(pulish).then(() => {
        ...
        inquirer.prompt([{
            ...
            execSync(`E:/bin/rsync/rsync -avz --progress --exclude-from="${path.join(__dirname, '..', 'extension', 'exclude.list')}" ./ bjbw@139.129.166.12::${mainConfig.publish.name}`, {
